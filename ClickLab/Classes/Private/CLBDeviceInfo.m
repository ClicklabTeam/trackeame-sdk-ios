...
@implementation CLBDeviceInfo {
    NSObject *networkInfo;
}

@synthesize appVersion = _appVersion;
@synthesize osVersion = _osVersion;
@synthesize osBrand = _osBrand;
...
- (NSString *)osBrand {
    if (!_osBrand) {
//        _osBrand = [[UIDevice currentDevice] systemName];
        // FIXME: limitation on the backend. Only accepts the value "ios". On iOS 8.1 the systemName is "iPhone OS".
        _osBrand = [kCLBOSName lowercaseString];
    }
    return _osBrand;
}

- (NSString *)manufacturer {
    return kCLBDeviceManufacturer;
}

- (NSString *)model {
    if (!_model) {
        _model = [CLBDeviceInfo getPhoneModel];
    }
    return _model;
}
...
- (NSString *)language {
    return [[NSLocale localeWithLocaleIdentifier:@"en_US"] displayNameForKey:NSLocaleLanguageCode value:[[NSLocale preferredLanguages] objectAtIndex:0]];
}
...
- (NSString *)advertiserID {
    if (!_advertiserID) {
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= (float) 6.0) {
            NSString *advertiserId = [CLBDeviceInfo getAdvertiserID:5];
            if (advertiserId != nil &&
...
- (NSString *)vendorID {
    if (!_vendorID) {
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= (float) 6.0) {
            NSString *identifierForVendor = [CLBDeviceInfo getVendorID:5];
...
- (NSString *)screenResolution {
    if (!_screenResolution) {
        ...
        _screenResolution = [NSString stringWithFormat:@"%.0fx%.0f", screenSize.width, screenSize.height];
    }
...
+ (NSString *)getAdvertiserID:(int)maxAttempts {
    ...
    if (ASIdentifierManager && sharedManager && advertisingIdentifier) {
        ...
        if (identifier == nil && maxAttempts > 0) {
            ...
            return [CLBDeviceInfo getAdvertiserID:maxAttempts - 1];
        } else {
            return identifier;
        }
    } else {
...
+ (NSString *)getVendorID:(int)maxAttempts {
    ...
    if (identifier == nil && maxAttempts > 0) {
        // Try again every 5 seconds
        [NSThread sleepForTimeInterval:5.0];
        return [CLBDeviceInfo getVendorID:maxAttempts - 1];
    } else {
        return identifier;
    }
}
...
+ (NSString *)generateUUID {
    ...
    NSString *result = [[CLBUtils generateUUID] stringByAppendingString:@"R"];
    return result;
}
